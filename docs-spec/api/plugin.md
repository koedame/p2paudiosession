...
| 機能 | 状態 |
|------|------|
| プラグインスキャン | 実装済（ファイル検出 + CLAPメタデータ取得） |
| CLAPプラグインロード | 実装済（libloadingによる動的ロード） |
| VST3プラグインロード | 未実装（プレースホルダー） |
| 音声処理 | 実装済（CLAPプラグイン） |
| パラメータ操作 | 実装済（CLAPプラグイン） |
| エディタUI | インターフェース定義済（未接続） |

VST3プラグインのロード機能には、VST3 SDKのインテグレーションが必要。
