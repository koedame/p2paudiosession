...
運用上の秘匿情報は `PrivateDocs/` ディレクトリに保存する：

- このディレクトリはGit管理対象外（.gitignoreで除外）
- 保存対象例：
  - APIトークン、認証情報
  - 非公開サーバーのアドレス・設定
  - 内部運用ドキュメント
...
- **GitHub Packages** (ghcr.io含む): Dockerイメージの配布にはGitHub Packagesを使用しない
  - 理由: プライベートなデプロイ環境を使用するため
  - Dockerイメージは各環境で直接ビルドするか、別の配布方法を使用する
